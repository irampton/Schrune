...
test("writes autogenerated BOM metadata into the imported schrune info block", async () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "schrune-lcsc-meta-"));
    const fetch = async (url) => {
        if (String(url).includes("/api/products/C1234/components")) {
            return response(easyEdaComponent);
        }
    ...
    try {
        ...
        const schrune = fs.readFileSync(path.join(dir, "parts", "ACME_123", "ACME_123.schrune"), "utf8");
        assert.match(schrune, /package: "0402"/);
        assert.match(schrune, /value: "1uF"/);
        assert.match(schrune, /tolerance: "±10%"/);
        assert.match(schrune, /maxVoltage: "10V"/);
        assert.match(schrune, /temperatureCoefficient: "X5R"/);
        assert.match(schrune, /comment: "1uF ±10% 10V X5R"/);
    } finally {
        fs.rmSync(dir, { recursive: true, force: true });
    }
});

test("updateExistingSchrune refreshes info fields without overwriting custom fields or pins", async () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "schrune-lcsc-update-"));
    const fetch = async (url) => {
        if (String(url).includes("/api/products/C1234/components")) {
            return response(easyEdaComponent);
        }
    ...
    try {
        ...
        fs.writeFileSync(path.join(partDir, "ACME_123.schrune"), `part ACME_123 {
    info: {
        partNumber: "OLD",
        manufacture: "Legacy",
        footprint: "./OLD.kicad_mod",
        symbol: "./OLD.kicad_sym",
        comment: "Keep me",
    }
...
`);
        ...
        await addLcscPart("C1234", { cwd: dir, fetch, bridgeRunner, updateExistingSchrune: true });

        const schrune = fs.readFileSync(path.join(partDir, "ACME_123.schrune"), "utf8");
        assert.match(schrune, /partNumber: "ACME-123"/);
        assert.match(schrune, /manufacture: "ACME"/);
        assert.match(schrune, /footprint: "\.\/ACME_123\.kicad_mod"/);
        assert.match(schrune, /symbol: "\.\/ACME_123\.kicad_sym"/);
        assert.match(schrune, /comment: "Keep me"/);
        assert.match(schrune, /CUSTOM:99/);
    } finally {
        fs.rmSync(dir, { recursive: true, force: true });
    }
});
